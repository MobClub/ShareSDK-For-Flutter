...
class ShareSDK 
{
   ...
   * 分享函数
   */
  static Future<dynamic> share(ShareSDKPlatform platform, SSDKMap params, Function(SSDKResponseState,Map,Map,SSDKError) result)
  {
   ...
   * 授权
   */
  static Future<dynamic> auth(ShareSDKPlatform platform, Map settings, Function(SSDKResponseState,Map,SSDKError) result)
  {
  ...
  static Future<dynamic> hasAuthed(int platform) async{
    return await _channel.invokeMethod(ShareSDKMethods.hasAuthed.name,platform);
  }
  ...
  static Future<dynamic> cancelAuth(int platform) async{
    return await _channel.invokeMethod(ShareSDKMethods.cancelAuth.name,platform);
  }

  /**
   * 获取用户信息
   */
  static Future<dynamic> getUserInfo(int platform, Function(SSDKResponseState,Map,SSDKError) result) async
  {
    Map args = {"platform":platform};
    Future<dynamic> callback = await _channel.invokeMethod(ShareSDKMethods.getUserInfo.name,args);
    callback.then((dynamic response){
      if (result != null)
      {
        result(_state(response),response["user"],SSDKError(rawData: response["error"]));
 ...
 * 弹出编辑菜单
 */
  static Future<dynamic> showEditor(ShareSDKPlatform platform,SSDKMap params,Function(SSDKResponseState,ShareSDKPlatform,Map,Map,SSDKError) result)
  {
    Map args = {"platform":platform.id,"params":params.map};
